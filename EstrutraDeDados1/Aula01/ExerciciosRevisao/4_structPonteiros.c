...
void criarBaralho(Baralho *baralho) {
    char valores[VALOR_SZ] = {'A', '2', '3', '4', '5', '6', '7', '8', '9', '0', 'J', 'Q', 'K'};
    char naipes[NAIPES_SZ] = {'P','O','E','C'};

    for (int i = 0; i < CARTAS_SZ; i++) {
        int iValor = i % 13;
        int iNaipe = i % 4;
        baralho->array[i].valor = valores[iValor];
        baralho->array[i].naipe = naipes[iNaipe];
...
Carta fundo(Baralho *baralho) {
    return baralho->array[CARTAS_SZ - CARTAS_SZ];
}
